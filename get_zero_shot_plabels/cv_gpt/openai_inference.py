...
def main():
    ...
    parser.add_argument('--self_checkpointing_steps', type=int, default=0)
    parser.add_argument('--self_checkpointing_exit_code', type=int, default=85)
    ...
    if os.path.exists(output_text_fname):
        ...
        output_texts = run_chat_completion_image_classification_openai(root_dir, image_file_list, target_labels, candidate_labels, topic, args)
        torch.save(output_texts, output_text_fname)
        print("saved output texts in ", output_text_fname)

    # ### evaluate outputs and save.
    pred_labels = my_img_decoded_prediction(args.task_name, output_texts, candidate_labels)
...
import base64
def run_chat_completion_image_classification_openai(root_dir, image_file_list, target_labels, candidate_labels, topic, args):
    ...
    print(f"--> Running chat completion zero-shot image classification with model {model_name} seed {seed} ")

    system_content = f"You are an image classification system, asked to classify {topic}.\n"
    system_content += "Classify the input image into exactly one of the following types:\n\n"+ ", ".join(candidate_labels)+"\n\n"
    system_content += "Respond with only the type that best fits the image."

    print("system prompt:")
    print(system_content)

    n_samples = len(image_file_list)
    ...
    for idx in range(last_idx+1, n_samples):

        img_fname = os.path.join(root_dir, image_file_list[idx])
        ...
        if args.self_checkpointing_steps > 0 and (idx+1) % args.self_checkpointing_steps == 0:
            print(f"self ckpt exit : steps {args.self_checkpointing_steps} exit code {args.self_checkpointing_exit_code}")
            sys.exit(args.self_checkpointing_exit_code)
