...
task_name=flowers # datasets: flowers, resisc
label_size=204 # number of labeled data 
...
main() {
    ...
    python -u train.py \
        ...
        --c config/usb_cv_framed_paper/fullysupervised/fullysupervised_${task_name}_${config_label_size}_paper_0.yaml \
    ...
    python -u train.py \
        ...
        --c config/usb_cv_framed_paper/${algo_name}/${algo_name}_${task_name}_${config_label_size}_paper_0.yaml \
        --data_dir_ow $data_path \
        --plabel_data_path "${data_path}/plabels/${plabel_exp_name}/plabel_results.pt" \
        --epoch_ow 4 --num_train_iter_ow 96 --num_eval_iter_ow 32 --num_log_iter_ow 8
...
set_config_label_size() {
    ...
    if [ "$task_name" == "flowers" ]; then
        config_label_size=400
    elif [ "$task_name" == "resisc" ]; then
        config_label_size=400
    else
...
set_plabel_exp_name() {
    ...
    if [ "$plabel_model" == "0" ]; then
        plabel_exp_name="zero_shot_${task_name}_clip-vit-large-patch14_train_p0_sd42"
    elif [ "$plabel_model" == "1" ]; then
        plabel_exp_name="zero_shot_${task_name}_clip-vit-base-patch32_train_p0_sd42"
    elif [ "$plabel_model" == "2" ]; then
        plabel_exp_name="zero_shot_${task_name}_gpt-4.1-2025-04-14_train_p0_sd42"
    else
