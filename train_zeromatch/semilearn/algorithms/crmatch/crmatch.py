...
class CRMatch_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        _, logits, p_logits = self.backbone(feat_maps, only_fc=True)

        # results_dict = {'logits':logits, 'logits_ds':logits_ds, 'feat':feat_maps}
        results_dict = {'logits':logits, 'logits_ds':logits_ds, 'feat':feat_maps, 'p_logits':p_logits}
...
class CRMatch(AlgorithmBase):
    ...
    def set_ema_model(self):
        ...
        ema_model = super().set_ema_model(load_state_dict=False)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            #### ckpt exit
            if self.args.self_checkpointing_epochs > 0:
                ckpt_exit_epochs = self.args.self_checkpointing_epochs
                ckpt_exit_code = self.args.self_checkpointing_exit_code
                if (self.epoch+1) % (ckpt_exit_epochs)  == 0:
                    import sys
                    print(f"self checkpoint exit: epoch+1 {(self.epoch+1)} ckpt_exit interval {ckpt_exit_epochs}, exit code {ckpt_exit_code}")
                    sys.exit(ckpt_exit_code)

        self.call_hook("after_run")


    def train_step(self, x_lb, y_lb, yp_lb, x_ulb_w, x_ulb_s, yp_ulb,  x_ulb_rot=None, rot_v=None):
        num_lb = y_lb.shape[0]
        ...
        with self.amp_cm():
            if self.use_cat:
                ...
                p_logits_x_ulb_w, p_logits_x_ulb_s = p_logits[num_lb:num_lb + 2 * num_ulb].chunk(2)

            else:
                outs_x_lb = self.model(x_lb)
                logits_x_lb = outs_x_lb['logits']
                p_logits_x_lb = outs_x_lb['p_logits']
                ...
                outs_x_ulb_w = self.model(x_ulb_w)
                logits_x_ulb_w, logits_ds_w, feats_x_ulb_w = outs_x_ulb_w['logits'], outs_x_ulb_w['logits_ds'], outs_x_ulb_w['feat']
                p_logits_x_ulb_w = outs_x_ulb_w['p_logits']
                logits_ds_w = logits_ds_w.detach()
            ...
            p_logits_x_all = torch.cat([p_logits_x_lb, p_logits_x_ulb_w])
            yp_all =  torch.cat([yp_lb, yp_ulb])
            psup_loss = self.ce_loss(p_logits_x_all, yp_all, reduction='mean')

            # total_loss = Lx + Lu + Ld
            lmbd = self.args.lmbd
            ann_alpha = self.anneal_scheduler.get_anneal_alpha()
            total_loss = Lx + Lu + Ld +  lmbd * ann_alpha * psup_loss
        ...
        log_dict = self.process_log_dict(sup_loss=Lx.item(), 
                                         ...
                                         psup_loss=psup_loss.item(), 
                                         ann_alpha = lmbd * ann_alpha.item())
