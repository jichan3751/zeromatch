...
class SimMatch_Net(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        _, logits, p_logits = self.backbone(feat, only_fc=True)
        if self.epass:
        ...
        return {'logits':logits, 'p_logits':p_logits, 'feat':feat_proj}
...
class SimMatch(AlgorithmBase):
    ...
    def set_model(self): 
        ...
        if not hasattr(self.args, 'use_epass'):
            self.args.use_epass = False
    ...
    def train_step(self, idx_lb, x_lb, y_lb, yp_lb, x_ulb_w, x_ulb_s, yp_ulb):
        num_lb = y_lb.shape[0]
        ...
        with self.amp_cm():
            ...
            if self.use_cat:
                ...
                p_logits_x_ulb_w, p_logits_x_ulb_s = p_logits[num_lb:].chunk(2)
                ...
                p_logits_x_ulb_s = outs_x_ulb_s['p_logits']
            ...
            total_loss = sup_loss + self.lambda_u * unsup_loss + self.lambda_in * in_loss + lmbd * ann_alpha * psup_loss
