...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    """
    from semilearn.datasets import get_eurosat, get_medmnist, get_semi_aves, get_cifar, get_svhn, get_stl10, get_imagenet, get_framed, get_json_dset, get_pkl_dset
    ...
    if dataset == "eurosat":
        ...
        test_dset = None
    elif dataset in ['dtd','flowers','resisc']:
        lb_dset, ulb_dset, eval_dset = get_framed(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb)
        test_dset = None
    # speech dataset
...
class AnnealScheduler:
    def __init__(self, optimizer, num_training_steps, alpha_init, alpha_end, mode = 1):
        ...
        # embed alph into optimizer param groups.
        for param_group in self.optimizer.param_groups:
            param_group.update({'anneal_init':alpha_init, 'anneal_alpha':alpha_init, 'anneal_step_count':0})

    def get_anneal_alpha(self):
        if self.alpha_end < 1e-99 :
            return torch.tensor(0)

        step_count = self.optimizer.param_groups[0]['anneal_step_count']

        tmp = torch.linspace(self.alpha_init, self.alpha_end, self.num_training_steps)

        # Debugging

        # if self._step_count-1 < len(tmp):
        #     alph = tmp[self._step_count-1]
        # else:
        #     print(f"len(tmp):{len(tmp)} step count idx {self._step_count-1}")
        #     alph = tmp[-1]

        if step_count > len(tmp) -1:
            print("WARNING: Anneal scheduler step_count: overflowed. setting to max..")
            step_count = len(tmp) -1
            
        alpha = tmp[step_count]
        # import ipdb; ipdb.set_trace()
        # print("al", alpha, step_count,self.num_training_steps )

        if self.mode == 0:
            # non annealing
            return torch.tensor(self.alpha_end)
        else:
            return alpha
    ...
    def step(self):
        ...
        alpha = self.get_anneal_alpha()
        for param_group in self.optimizer.param_groups:
            param_group.update({'anneal_alpha':alpha, 'anneal_step_count':step_count})

        # if step_count % 100 ==0:
        #     print(f"AnnealScheduler debug log: {step_count} update alpha {alpha} opt step {self.optimizer._step_count}")


def get_anneal_scheduler(optimizer, num_training_steps, mode):

    #### edit: also create alpha annealing scheduler
    anneal_scheduler_mode = mode # 1 : anneal, 0: fixed
    print("anneal_scheduler_mode", anneal_scheduler_mode)

    anneal_scheduler = AnnealScheduler(
        optimizer=optimizer,
        num_training_steps=num_training_steps,
        alpha_init = 0,
        alpha_end = 1.0,
        mode = anneal_scheduler_mode,
    )

    return anneal_scheduler
