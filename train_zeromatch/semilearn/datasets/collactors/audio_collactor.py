...
class DataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if 'labels' in batch:
            return {'idx_lb': batch['idx'].long(), 'x_lb': batch['input_values'], 'y_lb': batch['labels'].long(), 'yp_lb':batch['plabel']}
        else:
            if len(s_features) > 0:
                ...
                if len(s_features_) > 0:
                    ...
                    return {'idx_ulb': batch['idx'].long(), 'x_ulb_w': batch['input_values'], 'x_ulb_s_0': s_batch['input_values'], 'x_ulb_s_1': s_batch_['input_values'], 'yp_ulb':batch['plabel']}
                else:
                    return {'idx_ulb': batch['idx'].long(), 'x_ulb_w': batch['input_values'], 'x_ulb_s': s_batch['input_values'],'yp_ulb':batch['plabel']}
            else:
                return {'idx_ulb': batch['idx'].long(), 'x_ulb_w': batch['input_values'],'yp_ulb':batch['plabel']}
