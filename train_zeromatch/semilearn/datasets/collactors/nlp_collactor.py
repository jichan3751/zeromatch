...
class DataCollatorWithPadding:
    ...
    def __call__(self, features):
        ...
        if 'label' in batch:
            return {'idx_lb': batch['idx'], 'x_lb': {'input_ids': batch['input_ids'], 'attention_mask': batch['attention_mask']}, 'y_lb': batch['label'],'yp_lb':batch['plabel']}
        else:
            if len(s_features) > 0:
                ...
                if len(s_features_) > 0:
                    ...
                    return {'idx_ulb': batch['idx'], 'x_ulb_w': {'input_ids': batch['input_ids'], 'attention_mask': batch['attention_mask']}, 'x_ulb_s': {'input_ids': s_batch['input_ids'], 'attention_mask': s_batch['attention_mask']},'yp_ulb':batch['plabel']}
            else:
                return {'idx_ulb': batch['idx'], 'x_ulb_w': {'input_ids': batch['input_ids'], 'attention_mask': batch['attention_mask']},'yp_ulb':batch['plabel']}
