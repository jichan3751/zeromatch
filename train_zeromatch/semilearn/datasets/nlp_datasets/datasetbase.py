...
class BasicDatasetWithPLabel(Dataset):
    ...
    def __init__(self,
        ...
        assert len(self.plabels) == len(self.data)
    ...
    def __getitem__(self, idx):
        ...
        # set augmented images
        if self.is_ulb == False:
            return {'idx':idx, 'text':sen[0], 'label':target, 'plabel':plabel} 
        else:
            if self.alg == 'fullysupervised' or self.alg == 'supervised':
                return {'idx':idx, 'text':sen[0], 'label':target, 'plabel':plabel} 
            if self.alg == 'pseudolabel' or self.alg == 'vat':
                return {'idx':idx, 'text':sen[0], 'plabel':plabel} 
            elif self.alg == 'pimodel' or self.alg == 'meanteacher' or self.alg == 'mixmatch':
                return {'idx':idx, 'text':sen[0], 'text_s':sen[0], 'plabel':plabel}
            elif self.alg == 'comatch' or self.alg == 'remixmatch':
                indices = [1, 2]
                np.random.shuffle(indices)
                return {'idx':idx, 'text':sen[0], 'text_s':sen[indices[0]], 'text_s_':sen[indices[1]], 'plabel':plabel}
            else:
                return {'idx':idx, 'text':sen[0], 'text_s':sen[self.random_choose_sen()], 'plabel':plabel}
