...
def get_config():
    ...
    ###### my arguments to overwrite
    parser.add_argument("--data_dir_ow", type=str, default=None)
    parser.add_argument("--amp_ow", type=int, default=None)
    parser.add_argument("--load_path_ow", type=str, default=None)
    parser.add_argument("--save_dir_ow", type=str, default=None)
    parser.add_argument("--save_name_ow", type=str, default=None)
    parser.add_argument("--num_labels_ow", type=int, default=None)
    parser.add_argument("--seed_ow", type=int, default=None)
    
    parser.add_argument("--epoch_ow", type=int, default=None)
    parser.add_argument("--num_train_iter_ow", type=int, default=None)
    parser.add_argument("--num_warmup_iter_ow", type=int, default=None)
    parser.add_argument("--num_eval_iter_ow", type=int, default=None)
    parser.add_argument("--num_log_iter_ow", type=int, default=None)

    # parser.add_argument("--num_workers_ow", type=int, default=0)
    # print("setting num_workers to be 0000")

    parser.add_argument("--lr_ow", type=float, default=None)
    parser.add_argument("--net_ow", type=str, default=None)
    parser.add_argument("--use_pretrain_ow", default=None, type=str2bool)
    parser.add_argument("--pretrain_path_ow", type=str, default=None)
    parser.add_argument("--img_size_ow", type=int, default=None)
    
    parser.add_argument('--disable_multiprocessing_distributed', action='store_true')

    ## zeromatch-specific args.
    parser.add_argument('--plabel_data_path', type=str, default='')
    parser.add_argument('--ann_mode', type=int, default=1) # annealing mode
    ...
    parser.add_argument('--load_model_only_path', type=str, default='')
    ...
    ### overwrite arguments that ends with '_ow'
    args = overwrite_ow_args(args)

    return args


def overwrite_ow_args(args):
    
    args_d = vars(args)

    ow_arg_names = [arg_name for arg_name in args_d if arg_name.endswith("_ow")]

    for arg_name in ow_arg_names:
        arg_value = args_d[arg_name]
        if arg_value is not None:
            print("args overwrited:",arg_name, arg_value)
            setattr(args, arg_name[:-3], arg_value)

        # remove the dict
        args.__dict__.pop(arg_name)
    
    # import ipdb; ipdb.set_trace()

    if args.disable_multiprocessing_distributed:
        args.multiprocessing_distributed = False
    
    args.__dict__.pop("disable_multiprocessing_distributed")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    torch.cuda.manual_seed(args.seed) # added
    torch.cuda.manual_seed_all(args.seed) # added
    ...
    ## normally do not need this part. just for debugging.
    ...
    ## if loading from previous stage.
    if args.training_stage == 1:
        if os.path.exists(args.load_model_only_path):
            try:
                model.load_model_only(args.load_model_only_path)
            except:
                logger.info("Fail to resume load model only path {}".format(args.load_model_only_path))
        else:
            logger.info("args.training_stage == 1 but load model only path does not exist: {}. exitting...".format(args.load_model_only_path))
